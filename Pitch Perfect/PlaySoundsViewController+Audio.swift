...
extension PlaySoundsViewController: AVAudioPlayerDelegate {
    ...
    func setupAudio() {
        // initialize (recording) audio file from the recordsoundview controller
        do {
            audioFile = try AVAudioFile(forReading: recordedAudioURL)
        } catch {
            showAlert(Alerts.AudioFileError, message: String(error)) //if fail, show an alert statement to the user
        }
    ...
    func playSound(rate rate: Float? = nil, pitch: Float? = nil, echo: Bool = false, reverb: Bool = false) {
        ...
        if let pitch = pitch { //if patch it not nill, execute this statement
            changeRatePitchNode.pitch = pitch
        }
        if let rate = rate {// same as the pitch
